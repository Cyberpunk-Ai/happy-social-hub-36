...
create index if not exists post_impressions_post_idx on public.post_impressions (post_id);
...
create index if not exists space_messages_space_idx on public.space_messages (space_id, created_at);
...
drop trigger if exists t_payouts_updated on public.payouts;
...
drop trigger if exists t_monetization_updated on public.monetization_settings;
...
drop trigger if exists t_workspaces_updated on public.workspaces;
...
drop trigger if exists t_tickets_updated on public.support_tickets;
